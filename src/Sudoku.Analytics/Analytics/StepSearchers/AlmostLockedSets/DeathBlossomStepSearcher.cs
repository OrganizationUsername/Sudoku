...
public sealed partial class DeathBlossomStepSearcher : StepSearcher
{
	...
	protected internal override Step? Collect(ref StepAnalysisContext context)
	{
		...
		{
			if (BitOperations.PopCount((uint)playgroundCached[entryElimCell]) < 2)
			{
			...
			foreach (var wrongDigit in wrongDigitsMask)
			{
				...
				foreach (var availablePivotHouse in availablePivots.Houses)
				{
					...
					{
						if (BitOperations.PopCount((uint)playground[cell]) >= 2)
						{
					...
					for (satisfiedSize = 2; satisfiedSize <= 8; satisfiedSize++)
					{
						for (var a = 0; a <= preeliminationsCount; a++)
						{
							...
							for (var b = a + 1; b <= tempCount; b++)
							{
								...
								if (BitOperations.PopCount((uint)selectedCellDigitsMask[1]) < 2) { goto AlmostAlmostLockedSetDeletion; }
								...
								for (var c = b + 1; c <= tempCount; c++)
								{
									...
									if (BitOperations.PopCount((uint)selectedCellDigitsMask[2]) < 3) { goto AlmostAlmostLockedSetDeletion; }
									...
									for (var d = c + 1; d <= tempCount; d++)
									{
										...
										if (BitOperations.PopCount((uint)selectedCellDigitsMask[3]) < 4) { goto AlmostAlmostLockedSetDeletion; }
										...
										for (var e = d + 1; e <= tempCount; e++)
										{
											...
											if (BitOperations.PopCount((uint)selectedCellDigitsMask[4]) < 5) { goto AlmostAlmostLockedSetDeletion; }
											...
											for (var f = e + 1; f <= tempCount; f++)
											{
												...
												if (BitOperations.PopCount((uint)selectedCellDigitsMask[5]) < 6) { goto AlmostAlmostLockedSetDeletion; }
												...
												for (var g = f + 1; g <= tempCount; g++)
												{
													...
													if (BitOperations.PopCount((uint)selectedCellDigitsMask[6]) < 7) { goto AlmostAlmostLockedSetDeletion; }
													...
													for (var h = g + 1; h <= tempCount; h++)
													{
														...
														if (BitOperations.PopCount((uint)selectedCellDigitsMask[7]) < 8) { goto AlmostAlmostLockedSetDeletion; }
													}
												}
											}
										}
	...
	)
	{
		...
		Array.InitializeArray(detailViews, ([NotNull] ref view) => view = [new CellViewNode(ColorIdentifierAlias.Normal, pivot)]);

		var indexOfAls = 0;
		foreach (var digit in grid.GetCandidates(pivot))
		{
			var node = new CandidateViewNode(ColorIdentifierAlias.Auxiliary2, pivot * 9 + digit);
			candidateOffsets.Add(node);
			detailViews[indexOfAls++].Add(node);
		}

		// Collect for view nodes.
		(indexOfAls, var cellOffsets) = (0, (List<CellViewNode>)[new CellViewNode(ColorIdentifierAlias.Normal, pivot)]);
		foreach (var (branchDigit, (_, alsCells)) in branches)
		{
			foreach (var alsCell in alsCells)
			{
				var alsColor = ColorIdentifierAlias.AlmostLockedSet1 + indexOfAls;
				foreach (var digit in grid.GetCandidates(alsCell))
				{
					var node = new CandidateViewNode(
						branchDigit == digit
							? ColorIdentifierAlias.Auxiliary2
							: (zDigitsMask >> digit & 1) != 0 ? ColorIdentifierAlias.Auxiliary1 : alsColor,
						alsCell * 9 + digit
					);
					candidateOffsets.Add(node);
					detailViews[indexOfAls].Add(node);
	...
	)
	{
		...
		foreach (var disappearedDigit in disappearedDigitsMask)
		{
			...
			var candidateOffsets = new List<CandidateViewNode>();
			var houseOffset = new HouseViewNode(ColorIdentifierAlias.Normal, house);
			var detailViews = new View[branches.Count];
			var i = 0;
			foreach (ref var view in detailViews.AsSpan())
			{
				var candidate = targetCells[i++] * 9 + disappearedDigit;
				view = [houseOffset, new CandidateViewNode(ColorIdentifierAlias.Auxiliary2, candidate)];
			}
			...
			var step = new HouseDeathBlossomStep(
				...
				[
					[
						...
						select new CandidateViewNode(ColorIdentifierAlias.Auxiliary2, cell * 9 + disappearedDigit),
						.. candidateOffsets,
						houseOffset
					],
					.. detailViews
	...
	)
	{
		...
		for (var usedAlsIndex = 1; usedAlsIndex <= usedAlsesCount; usedAlsIndex++)
		{
			...
			for (var currentDigit = 0; currentDigit < 9; currentDigit++)
			{
				...
				foreach (var cell in alsesUsed[usedAlsIndex * 9 + currentDigit])
				{
					...
					{
						var candidateNode = new CandidateViewNode(ColorIdentifierAlias.Auxiliary2, cell * 9 + currentDigit);
					...
					var node = new CellViewNode(ColorIdentifierAlias.Normal, cell);
					view.Add(node);
			...
			{
				var cellNode = new CellViewNode(ColorIdentifierAlias.AlmostLockedSet1 + alsIndex, cell);
				view.Add(cellNode);
				cellOffsets.Add(cellNode);
					...
					var candidateNode = new CandidateViewNode(
						(rcc >> digit & 1) != 0
							...
							: (zDigitsMask >> digit & 1) != 0
								...
								: ColorIdentifierAlias.AlmostLockedSet1 + alsIndex,
						cell * 9 + digit
					);
					view.Add(candidateNode);
					candidateOffsets.Add(candidateNode);
		...
		var step = new NTimesAlmostLockedSetsDeathBlossomStep(
			...
			nTimesAlsCells,
			branches,
			BitOperations.PopCount((uint)grid[nTimesAlsCells]) - nTimesAlsCells.Count
		);
		if (context.OnlyFindOne)
		{
			return step;
