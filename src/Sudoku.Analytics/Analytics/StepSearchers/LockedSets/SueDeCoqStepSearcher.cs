						...
						foreach (ref readonly var currentBlockMap in blockMap & i)
						{
							...
							for (var j = 1; j <= 9 - i - currentInterMap.Count && j <= lineMap.Count; j++)
							{
								...
								foreach (ref readonly var currentLineMap in lineMap & j)
								{
									...
									var p = BitOperations.PopCount((uint)blockMask)
										+ BitOperations.PopCount((uint)lineMask)
										+ BitOperations.PopCount((uint)maskOnlyInInter);
									...
									foreach (var cell in currentBlockMap)
									{
										foreach (var digit in grid.GetCandidates(cell))
										{
											candidateOffsets.Add(
												new(
													!cannibalMode && digit == digitIsolated ? ColorIdentifierAlias.Auxiliary2 : ColorIdentifierAlias.Normal,
													cell * 9 + digit
												)
											);
										}
									...
									foreach (var cell in currentLineMap)
									{
										foreach (var digit in grid.GetCandidates(cell))
										{
											candidateOffsets.Add(
												new(
													!cannibalMode && digit == digitIsolated ? ColorIdentifierAlias.Auxiliary2 : ColorIdentifierAlias.Auxiliary1,
													cell * 9 + digit
												)
											);
										}
											...
											candidateOffsets.Add(
												new(
													digitIsolated == digit
														...
														: (blockMask >> digit & 1) != 0 ? ColorIdentifierAlias.Normal : ColorIdentifierAlias.Auxiliary1,
													cell * 9 + digit
												)
											);
										}
									...
									var step = new SueDeCoqStep(
										...
										[
											[
												.. candidateOffsets,
												new HouseViewNode(ColorIdentifierAlias.Normal, coverSet),
												new HouseViewNode(ColorIdentifierAlias.Auxiliary2, baseSet)
											]
										],
										context.Options,
										coverSet,
