	...
	where TSelf : ISkipMethod<TSelf, TSource>, allows ref struct
{
	...
	IEnumerable<TSource> Skip(int count) => new List<TSource>(this)[count..];

	/// <inheritdoc cref="Enumerable.SkipLast{TSource}(IEnumerable{TSource}, int)"/>
	IEnumerable<TSource> SkipLast(int count) => new List<TSource>(this)[..^count];

	/// <inheritdoc cref="Enumerable.SkipWhile{TSource}(IEnumerable{TSource}, Func{TSource, bool})"/>
	IEnumerable<TSource> SkipWhile(Func<TSource, bool> predicate)
	{
