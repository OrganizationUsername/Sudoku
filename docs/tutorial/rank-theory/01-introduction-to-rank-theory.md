...
> 可以看到，强区域和弱区域的定义是不对称的，这一点看起来和强弱链关系的定义不一样。从理解上讲，一个是最多 1 个（≤ 1），一个是必须填一个（= 1）确实符合人的理解一些。不过从秩理论整体系统设计上讲，强区域的确切定义确实是要求最少能填一个数（≥ 1）。
>
> 是不是很匪夷所思？对上面的例子而言（包括后续大多数时候而言）我们总可以得到客观层面一个区域必须只能最多填一个的状态（再多本身就容纳不下，直接违背数独规则），因而不可能出现填两次及更多的情况，但秩理论在设计上确实允许在同一个强区域里填入两个及以上的数字。这很不符合理解，反而会给读者造成困扰。本教程秩理论板块的前期都不会采用这个说法；后续会针对对结构整体分析秩的时候才会出现此说法的定义。
...
$$
r(结构)=n_{\text{max}}-n_{\text{fact}}
$$

我们把结构的秩记作 $$r(\text{结构})$$；当不强调具体结构时也可以直接记作字母 $$r$$（大写 $$R$$ 也可）。其中， $$n_{\text{max}}$$ 表示最多可填的次数，而 $$n_{\text{fact}}$$ 表示实际可填的次数。
...
这个人来自美国，他叫**罗伯特・艾伦・巴克尔**（Allan Barker），他早在 2009 年左右就已经对秩理论完备进行了框架设计，并且实现成了软件，就是前面介绍的 XSudo。
