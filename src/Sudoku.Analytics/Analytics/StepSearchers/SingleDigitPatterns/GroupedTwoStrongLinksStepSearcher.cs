			...
			for (var h1 = 0; h1 < 26; h1++)
			{
				for (var h2 = h1 + 1; h2 < 27; h2++)
				{
					...
					))
					{
						...
						if ((cells1 | cells2).FirstSharedHouse is not (var weakLinkHouse and not FallbackConstants.@int)
							|| !!(cells1 & cells2))
						{
							continue;
						}
						...
						var step = new TwoStrongLinksStep(
							...
							[
								[
									.. from cell in headCells select new CandidateViewNode(ColorIdentifierAlias.Normal, cell * 9 + digit),
									.. from cell in tailCells select new CandidateViewNode(ColorIdentifierAlias.Normal, cell * 9 + digit),
									.. from cell in cells1 select new CandidateViewNode(ColorIdentifierAlias.Auxiliary1, cell * 9 + digit),
									.. from cell in cells2 select new CandidateViewNode(ColorIdentifierAlias.Auxiliary1, cell * 9 + digit),
									new HouseViewNode(ColorIdentifierAlias.Normal, h1),
									new HouseViewNode(ColorIdentifierAlias.Normal, h2),
									new HouseViewNode(ColorIdentifierAlias.Auxiliary1, weakLinkHouse)
								]
							],
							context.Options,
							digit,
