	...
	MultipleForcingChains(conclusions)
{
	...
	)
	{
		...
		{
			var node = new CandidateViewNode(ColorIdentifierAlias.Auxiliary1, candidate);
	...
	protected override ReadOnlySpan<ViewNode> GetInitialViewNodes(in Grid grid)
		=> from candidate in TrueCandidates select (ViewNode)new CandidateViewNode(ColorIdentifierAlias.Auxiliary1, candidate);
}
