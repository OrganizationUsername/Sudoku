				...
				for (var size = BitOperations.PopCount((uint)otherDigitsMask) - 1; size < otherCells.Count; size++)
				{
					foreach (ref readonly var cells in otherCells & size)
					{
						...
						var step = new UniqueLoopType3Step(
							conclusions.AsMemory(),
							[[.. candidateOffsets, new HouseViewNode(ColorIdentifierAlias.Normal, houseIndex), .. GetLoopLinks(path)]],
							context.Options,
							d1,
							d2,
							loop,
			...
			foreach (var (digit, otherDigit) in ((d1, d2), (d2, d1)))
			{
				...
				var step = new UniqueLoopType4Step(
					...
					[
						[
							.. candidateOffsets,
							new ConjugateLinkViewNode(ColorIdentifierAlias.Normal, first, second, digit),
							.. GetLoopLinks(path)
						]
					],
					context.Options,
					...
					var step = new UniqueLoopConjugatePairsTypeStep(
						...
						[
							[
								...
								let id = cp.Digit == digit ? ColorIdentifierAlias.Normal : ColorIdentifierAlias.Auxiliary1
