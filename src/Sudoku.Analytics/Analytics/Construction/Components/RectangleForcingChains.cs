	...
	MultipleForcingChains(conclusions)
{
	...
	)
	{
		...
		{
			var node = new CellViewNode(ColorIdentifierAlias.Rectangle1, cell);
			...
			{
				var candidateNode = new CandidateViewNode(ColorIdentifierAlias.Rectangle1, cell * 9 + digit);
				foreach (var view in views)
				{
	...
	protected override ReadOnlySpan<ViewNode> GetInitialViewNodes(in Grid grid)
	{
		...
		foreach (var cell in Cells)
		{
			result.Add(new CellViewNode(ColorIdentifierAlias.Rectangle1, cell));
			foreach (var digit in UrDigitsMask & grid.GetCandidates(cell))
			{
				result.Add(new CandidateViewNode(ColorIdentifierAlias.Rectangle1, cell * 9 + digit));
			}
		}
