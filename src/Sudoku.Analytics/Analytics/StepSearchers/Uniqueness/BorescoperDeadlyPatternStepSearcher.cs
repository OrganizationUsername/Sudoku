	...
	)
	{
		...
		foreach (var digits in orMask.AllSets & (pattern.IsHeptagon ? 3 : 4))
		{
			...
			foreach (var cell in map)
			{
				foreach (var digit in grid.GetCandidates(cell))
				{
					candidateOffsets.Add(new(digit == otherDigit ? ColorIdentifierAlias.Auxiliary1 : ColorIdentifierAlias.Normal, cell * 9 + digit));
				}
			}
				...
				for (var size = BitOperations.PopCount((uint)otherDigitsMask) - 1; size < iterationCellsMap.Count; size++)
				{
					foreach (ref readonly var combination in iterationCellsMap & size)
					{
						...
						var step = new BorescoperDeadlyPatternType3Step(
							...
							[[.. candidateOffsets, new HouseViewNode(ColorIdentifierAlias.Normal, houseIndex)]],
							context.Options,
							map,
							tempMask,
							combination,
