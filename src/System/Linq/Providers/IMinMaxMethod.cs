	...
	where TSource : IComparable<TSource>, IComparisonOperators<TSource, TSource, bool>, IMinMaxValue<TSource>
{
	...
	TSource? Min() => Min(default(IComparer<TSource>));

	/// <inheritdoc/>
	TSource Min(Comparison<TSource> comparer) => Min(Comparer<TSource>.Create(comparer));
	...
	TSource MinBy<TKey>(Func<TSource, TKey> keySelector) => MinBy(keySelector, null);
	...
	TSource? Max() => Max(default(IComparer<TSource>));

	/// <inheritdoc/>
	TSource Max(Comparison<TSource> comparer) => Max(Comparer<TSource>.Create(comparer));
	...
	TSource MaxBy<TKey>(Func<TSource, TKey> keySelector) => MaxBy(keySelector, null);
