...
) : CreatorBase<ILinkViewNode, Shape>(pane, converter)
{
	...
	public override ReadOnlySpan<Shape> CreateShapes(ReadOnlySpan<ILinkViewNode> nodes)
	{
		...
		foreach (var node in nodes)
		{
			...
			var candidatePointInfoToDraw = new List<(double, Point, Point)>();
			...
			switch (node.Shape)
			{
				case LinkShape.Chain or LinkShape.ConjugatePair:
				{
					...
					var (distance, p, q) = (double.MaxValue, default(Point), default(Point));
					foreach (var s in startCandidates)
					{
						...
						foreach (var e in endCandidates)
						{
							...
							if (d <= distance)
							{
								(distance, p, q) = (d, tempPoint1, tempPoint2);
							}
						}
					}
					candidatePointInfoToDraw.Add((distance, p, q));
					break;
				}
				case LinkShape.Cell:
				{
					var p = Converter.GetPosition((Cell)start * 9 + 4);
					var q = Converter.GetPosition((Cell)end * 9 + 4);
					candidatePointInfoToDraw.Add((p.DistanceTo(q), p, q));
					break;
				}
				case LinkShape.RankSet when node is RankSetViewNode { Space: { Digit: var digit } space }:
				{
					var grid = Pane.Puzzle;
					var candidates = space.GetAvailableRange(grid);
					if (space.House < 9)
					{
						// Block.
						var cellPath = BlockCellConnection.GetConnections(candidates.Cells);
						for (var i = 0; i < cellPath.Length - 1; i++)
						{
							var s = cellPath[i] * 9 + digit;
							var e = cellPath[i + 1] * 9 + digit;
							var tempPoint1 = Converter.GetPosition(s);
							if (shouldRotate)
							{
								var margin = App.MiscellaneousRotatedCandidateItemsTranslationVectors[digit];
							...
							candidatePointInfoToDraw.Add((tempPoint1.DistanceTo(tempPoint2), tempPoint1, tempPoint2));
						}
					}
					else
					{
			...
			foreach (var (distance, p, q) in candidatePointInfoToDraw)
			{
				var (pt1, pt2) = (p, q);
				...
				else
				{
					...
					var strokeThicknessFactor = node switch
					{
						RankSetViewNode { Space: { IsHouseRelated: true, House: < 9 }, IsTruth: false } => 3.5,
						RankSetViewNode { Space.IsHouseRelated: true, IsTruth: false } => 3,
						{ Shape: LinkShape.ConjugatePair } => 2,
						{ Shape: LinkShape.RankSet } => 1.5,
						_ => 1
					};
					result.Add(
						new Path
							...
							Stroke = new SolidColorBrush(
								node switch
								{
									...
									RankSetViewNode { IsTruth: var isTruth, Space: var space } => (isTruth, space) switch
									{
										...
										{
											< 9 => Pane.BlockTruthColor,
											< 18 => Pane.RowTruthColor,
											_ => Pane.ColumnTruthColor
										},
										(_, { House: < 9 }) => Pane.BlockLinkColor,
										_ => Pane.LineOrCellLinkColor
									}
								}
							),
							StrokeThickness = (double)Pane.ChainStrokeThickness * strokeThicknessFactor,
							StrokeDashArray = dashArray,
