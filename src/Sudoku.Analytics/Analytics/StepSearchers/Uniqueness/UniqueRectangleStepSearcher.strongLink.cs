						...
						foreach (var urCell in urCells)
						{
							if (urCell == corner1 || urCell == corner2)
							{
								...
								if ((coveredHouses >> house & 1) != 0)
								{
									foreach (var d in grid.GetCandidates(urCell))
									{
										candidateOffsets.Add(
											new(
												d == digit ? ColorIdentifierAlias.Auxiliary1 : ColorIdentifierAlias.Normal,
												urCell * 9 + d
											)
										);
									}
								...
								else
								{
									foreach (var d in grid.GetCandidates(urCell))
									{
										candidateOffsets.Add(new(ColorIdentifierAlias.Normal, urCell * 9 + d));
									}
								}
							}
										...
										candidateOffsets.Add(
											new(
												urCell == elimCell
													...
													: d1 == digit ? ColorIdentifierAlias.Auxiliary1 : ColorIdentifierAlias.Normal,
												urCell * 9 + d1
											)
										);
									}
									...
									if (urCell != elimCell || d2 != elimDigit)
									{
										candidateOffsets.Add(
											new(
												urCell == elimCell
													? ColorIdentifierAlias.Normal
													: d2 == digit ? ColorIdentifierAlias.Auxiliary1 : ColorIdentifierAlias.Normal,
												urCell * 9 + d2
											)
										);
									}
							...
							new UniqueRectangleConjugatePairStep(
								...
								[
									[
										...
										.. candidateOffsets,
										new ConjugateLinkViewNode(ColorIdentifierAlias.Normal, cell, otherCell, digit)
									]
								],
								context.Options,
	...
	private partial void Check3X2SL(SortedSet<UniqueRectangleStep> accumulator, in Grid grid, ref StepAnalysisContext context, Cell[] urCells, bool arMode, Mask comparer, Digit d1, Digit d2, Cell cornerCell, in CellMap otherCellsMap, int index)
	{
		...
		foreach (var (a, b) in ((d1, d2), (d2, d1)))
		{
			...
			foreach (var digit in grid.GetCandidates(abxCell))
			{
				...
				{
					candidateOffsets.Add(new(digit == b ? ColorIdentifierAlias.Auxiliary1 : ColorIdentifierAlias.Normal, abxCell * 9 + digit));
				}
			}
			foreach (var digit in grid.GetCandidates(abyCell))
			{
				...
				{
					candidateOffsets.Add(new(digit == a ? ColorIdentifierAlias.Auxiliary1 : ColorIdentifierAlias.Normal, abyCell * 9 + digit));
				}
			}
			foreach (var digit in grid.GetCandidates(abzCell))
			{
				if (digit == a || digit == b)
				{
					candidateOffsets.Add(new(ColorIdentifierAlias.Auxiliary1, abzCell * 9 + digit));
				}
			}
			foreach (var digit in comparer)
			{
				candidateOffsets.Add(new(ColorIdentifierAlias.Normal, cornerCell * 9 + digit));
			}
				...
				new UniqueRectangleConjugatePairStep(
					...
					[
						[
							.. arMode ? GetHighlightCells(urCells) : [],
							.. candidateOffsets,
							new ConjugateLinkViewNode(ColorIdentifierAlias.Normal, abxCell, abzCell, b),
							new ConjugateLinkViewNode(ColorIdentifierAlias.Normal, abyCell, abzCell, a)
						]
					],
					context.Options,
	...
	private partial void Check3N2SL(SortedSet<UniqueRectangleStep> accumulator, in Grid grid, ref StepAnalysisContext context, Cell[] urCells, bool arMode, Mask comparer, Digit d1, Digit d2, Cell cornerCell, in CellMap otherCellsMap, int index)
	{
		...
		foreach (var (begin, end) in ((abxCell, abyCell), (abyCell, abxCell)))
		{
			...
			foreach (var (a, b) in digitPairs)
			{
				...
				{
					candidateOffsets.Add(new(d == a ? ColorIdentifierAlias.Auxiliary1 : ColorIdentifierAlias.Normal, cornerCell * 9 + d));
				}
				foreach (var d in digits)
				{
					if (CandidatesMap[d].Contains(abzCell))
					{
						candidateOffsets.Add(new(d == b ? ColorIdentifierAlias.Auxiliary1 : ColorIdentifierAlias.Normal, abzCell * 9 + d));
					}
				}
				foreach (var d in grid.GetCandidates(begin))
				{
					if (d == d1 || d == d2)
					{
						candidateOffsets.Add(new(ColorIdentifierAlias.Auxiliary1, begin * 9 + d));
					}
				}
				foreach (var d in grid.GetCandidates(end))
				{
					if ((d == d1 || d == d2) && d != a)
					{
						candidateOffsets.Add(new(ColorIdentifierAlias.Normal, end * 9 + d));
					}
				}
				if (!AllowIncompleteUniqueRectangles && candidateOffsets.Count != 7)
				{
				...
				accumulator.Add(
					new UniqueRectangleConjugatePairStep(
						Array.Single(new Conclusion(Elimination, end, a)),
						[
							[
								.. arMode ? GetHighlightCells(urCells) : [],
								.. candidateOffsets,
								new ConjugateLinkViewNode(ColorIdentifierAlias.Normal, cornerCell, begin, a),
								new ConjugateLinkViewNode(ColorIdentifierAlias.Auxiliary1, begin, abzCell, b)
							]
						],
						context.Options,
		...
		foreach (var (begin, end) in ((abxCell, abyCell), (abyCell, abxCell)))
		{
			...
			foreach (var (a, b) in ((d1, d2), (d2, d1)))
			{
				...
				foreach (var d in grid.GetCandidates(end))
				{
					if (d == d1 || d == d2)
					{
						candidateOffsets.Add(new(d == a ? ColorIdentifierAlias.Auxiliary1 : ColorIdentifierAlias.Normal, end * 9 + d));
					}
				}
				...
				accumulator.Add(
					new UniqueRectangleConjugatePairStep(
						Array.Single(new Conclusion(Elimination, begin, a)),
						[
							[
								.. arMode ? GetHighlightCells(urCells) : [],
								.. candidateOffsets,
								new ConjugateLinkViewNode(ColorIdentifierAlias.Normal, cornerCell, end, a),
								new ConjugateLinkViewNode(ColorIdentifierAlias.Auxiliary1, begin, abzCell, b)
							]
						],
						context.Options,
		...
		foreach (var (begin, end) in ((abxCell, abyCell), (abyCell, abxCell)))
		{
			...
			foreach (var (a, b) in ((d1, d2), (d2, d1)))
			{
				...
				foreach (var d in grid.GetCandidates(begin))
				{
					if (d == d1 || d == d2)
					{
						candidateOffsets.Add(new(d == a ? ColorIdentifierAlias.Auxiliary1 : ColorIdentifierAlias.Normal, begin * 9 + d));
					}
				}
				foreach (var d in grid.GetCandidates(end))
				{
				...
				foreach (var d in grid.GetCandidates(abzCell))
				{
					if ((d == d1 || d == d2) && d != b)
					{
						candidateOffsets.Add(new(d == a ? ColorIdentifierAlias.Auxiliary1 : ColorIdentifierAlias.Normal, abzCell * 9 + d));
					}
				}
				if (!AllowIncompleteUniqueRectangles && candidateOffsets.Count != 7)
				{
				...
				accumulator.Add(
					new UniqueRectangleConjugatePairStep(
						Array.Single(new Conclusion(Elimination, abzCell, b)),
						[
							[
								.. arMode ? GetHighlightCells(urCells) : [],
								.. candidateOffsets,
								new ConjugateLinkViewNode(ColorIdentifierAlias.Normal, cornerCell, end, a),
								new ConjugateLinkViewNode(ColorIdentifierAlias.Auxiliary1, begin, abzCell, a)
							]
						],
						context.Options,
		...
		foreach (var (a, b) in ((d1, d2), (d2, d1)))
		{
			...
			foreach (var (head, begin, end, extra) in ((corner2, corner1, abzCell, abwCell), (corner1, corner2, abwCell, abzCell)))
			{
				...
				foreach (var d in grid.GetCandidates(head))
				{
					if ((d == d1 || d == d2) && d != b)
					{
						candidateOffsets.Add(new(ColorIdentifierAlias.Auxiliary1, head * 9 + d));
					}
				}
				foreach (var d in grid.GetCandidates(extra))
				{
					if ((d == d1 || d == d2) && d != b)
					{
						candidateOffsets.Add(new(ColorIdentifierAlias.Auxiliary1, extra * 9 + d));
					}
				}
				foreach (var d in grid.GetCandidates(begin))
				{
				...
				foreach (var d in grid.GetCandidates(end))
				{
					if (d == d1 || d == d2)
					{
						candidateOffsets.Add(new(ColorIdentifierAlias.Auxiliary1, end * 9 + d));
					}
				}
					...
					new UniqueRectangleConjugatePairStep(
						...
						[
							[
								.. arMode ? GetHighlightCells(urCells) : [],
								.. candidateOffsets,
								new ConjugateLinkViewNode(ColorIdentifierAlias.Normal, head, begin, a),
								new ConjugateLinkViewNode(ColorIdentifierAlias.Auxiliary1, begin, end, b),
								new ConjugateLinkViewNode(ColorIdentifierAlias.Normal, end, extra, a)
							]
						],
						context.Options,
				...
				for (var i = 0; i < 2; i++)
				{
					...
					foreach (var d in grid.GetCandidates(abx))
					{
						if (d == d1 || d == d2)
						{
							candidateOffsets.Add(
								new(
									i == 0 ? d == a ? ColorIdentifierAlias.Auxiliary1 : ColorIdentifierAlias.Normal : ColorIdentifierAlias.Auxiliary1,
									abx * 9 + d
								)
							);
						}
					...
					foreach (var d in grid.GetCandidates(abz))
					{
						if (d == d1 || d == d2)
						{
							candidateOffsets.Add(new(d == b ? ColorIdentifierAlias.Auxiliary1 : ColorIdentifierAlias.Normal, abz * 9 + d));
						}
					}
					foreach (var d in grid.GetCandidates(aby))
					{
						if ((d == d1 || d == d2) && d != b)
						{
							candidateOffsets.Add(new(ColorIdentifierAlias.Auxiliary1, aby * 9 + d));
						}
					}
					foreach (var d in grid.GetCandidates(abw))
					{
						if (d == d1 || d == d2)
						{
							candidateOffsets.Add(new(ColorIdentifierAlias.Auxiliary1, abw * 9 + d));
						}
					}
					if (!AllowIncompleteUniqueRectangles && candidateOffsets.Count != 7)
					{
					...
					accumulator.Add(
						new UniqueRectangleConjugatePairStep(
							Array.Single(new Conclusion(Elimination, aby, b)),
							[
								[
									.. arMode ? GetHighlightCells(urCells) : [],
									.. candidateOffsets,
									new ConjugateLinkViewNode(ColorIdentifierAlias.Normal, abx, aby, a),
									new ConjugateLinkViewNode(ColorIdentifierAlias.Normal, aby, abw, a),
									new ConjugateLinkViewNode(ColorIdentifierAlias.Auxiliary1, linkMap[0], linkMap[1], b)
								]
							],
							context.Options,
