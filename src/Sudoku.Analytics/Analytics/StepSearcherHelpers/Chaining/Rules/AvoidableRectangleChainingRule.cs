...
public sealed class AvoidableRectangleChainingRule : ChainingRule
{
	...
	{
		var urIndex = processedViewNodesMap.MaxKeyInRectangle is var key and not ColorIdentifierAlias.Normal
			? (key - ColorIdentifierAlias.Rectangle1 + 1) % 3
			: 0;
		...
		foreach (var link in pattern.Links)
		{
			...
			var id = urIndex + ColorIdentifierAlias.Rectangle1;
			foreach (var cell in urCells)
			{
				if (view.FindCell(cell) is { Identifier: var originalIdentifier } cellViewNode)
				{
					if (originalIdentifier is (_, >= ColorIdentifierAlias.Rectangle1 and <= ColorIdentifierAlias.Rectangle3))
					{
