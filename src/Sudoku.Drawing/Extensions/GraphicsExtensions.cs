...
public static class GraphicsExtensions
{
	...
	extension(Graphics @this)
	{
		...
		/// </exception>
		public void DrawHollowArrow(Brush brush, PointF center, float length, float width, float height, float angle)
		{
			...
			// Rotating.
			var oldMatrix = @this.Transform;
			using var newMatrix = @this.Transform.Clone();
		...
		/// <exception cref="ArgumentException">
		/// Throws when <paramref name="circleRadius"/> is greater than the value in <paramref name="rectangle"/>.
		/// </exception>
		public void DrawRoundedRectangle(Pen pen, RectangleF rectangle, float circleRadius)
		{
			var (x, y, w, h) = rectangle;
			var (l, _) = rectangle;

			ArgumentException.ThrowIf(circleRadius < Max(w, h));
		...
		/// Throws when <paramref name="circleRadius"/> is greater than the value in <paramref name="rectangle"/>.
		/// </exception>
		public void FillRoundedRectangle(Brush brush, RectangleF rectangle, float circleRadius)
		{
			var (x, y, w, h) = rectangle;
			var (l, _) = rectangle;
