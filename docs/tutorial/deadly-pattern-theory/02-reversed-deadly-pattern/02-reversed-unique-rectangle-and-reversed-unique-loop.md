...
如果我们让 `r3c78` 里只有 1 和 7 的话，那么显然 1 和 7 在全盘所有明数的位置，配合 `r3c78` 就会构成唯一环的摆放模式，于是会造成矛盾。

但是，显然 `r3c78` 又不会出现都填 8 和 9 的情况（否则 `r3c5` 没办法填数），所以只能 `r3c78` 里拿一个格子填 8 或 9，然后和 `r3c5` 配合在一起形成待定显性数对，得到 `r3c3 <> 9` 的结论。
...
它的理解方式是这样的：若干让 `r2c9` 和 `r7c2` 同时都不填 9，那么这两个格子就只能填 4，融入到结构里就会构成两个唯一环的摆放模式。虽然是分离的、独立的两个唯一环，但是因为他们使用的行、列、宫互相都不影响彼此，所以不管你分开看还是合一起看，我们都能说它能造成余下空格的 2 和 4 无法区分填数情况。所以这照样可以造成矛盾。所以，结论就是 `{r2c2, r7c9} <> 9`。
