...
public sealed partial class NormalFishStepSearcher : StepSearcher
{
	...
	)
	{
		...
		for (var digit = 0; digit < 9; digit++)
		{
			...
			foreach (var bs in baseSetsSpan[1..(baseSetsSpan.IndexOf(0) is var i and not -1 ? i : ^0)] & size)
			{
				...
				foreach (var cs in coverSetsSpan[1..(coverSetsSpan.IndexOf(0) is var j and not -1 ? j : ^0)] & size)
				{
			...
			foreach (var cell in HousesMap[baseSet])
			{
				switch (CandidatesMap[digit].Contains(cell))
				{
					...
					default:
					{
						...
						{
							if (HousesMap[c.GetHouse(searchRow ? HouseType.Column : HouseType.Row)].Contains(cell))
							{
								flag = true;
								break;
							}
