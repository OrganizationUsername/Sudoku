	...
	where TSelf : IZipMethod<TSelf, TSource>, allows ref struct
{
	...
	IEnumerable<(TSource First, TSecond Second)> Zip<TSecond>(IEnumerable<TSecond> second)
	...
	IEnumerable<(TSource First, TSecond Second, TThird Third)> Zip<TSecond, TThird>(IEnumerable<TSecond> second, IEnumerable<TThird> third)
		=> Zip(second, third, static (first, second, third) => (first, second, third));

	/// <inheritdoc cref="Enumerable.Zip{TFirst, TSecond, TResult}(IEnumerable{TFirst}, IEnumerable{TSecond}, Func{TFirst, TSecond, TResult})"/>
	IEnumerable<TResult> Zip<TSecond, TResult>(IEnumerable<TSecond> second, Func<TSource, TSecond, TResult> resultSelector)
	{
	...
	/// <inheritdoc/>
	IEnumerable<TResult> Zip<TSecond, TThird, TResult>(IEnumerable<TSecond> second, IEnumerable<TThird> third, Func<TSource, TSecond, TThird, TResult> resultSelector)
	{
		var result = new List<TResult>();
		using var e1 = GetEnumerator();
		using var e2 = second.GetEnumerator();
