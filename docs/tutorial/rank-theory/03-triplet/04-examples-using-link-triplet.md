...
这个题虽然有 8 个弱区域，但实际上就能填 6 个数字进去。而且前面的内容已经充分说明这个题只能也必须填刚好 6 个数字进去，所以这个题的最多填数次数是 6，最少也是 6，故这个结构的秩为 0。虽然它是零秩的，但它不代表所有弱区域都能删数，主要是因为我们用的是新的公式算出来的，而它包含弱三元组，属于特殊覆盖规则，就必须用新的公式而不能套用早期的算法来算这个结构的秩。
