...
我们将四个目标单元格 `r13c4` 和 `r2c37` 结合起来，他们整体只能填 $$a$$、$$b$$、$$c$$ 和 $$d$$ 四个数，而且还互不相同。这便是这个结合起来产生的新结论：四个目标单元格整体构成一个跨区四数组。那么，结论呢？结论就是，`r2c56` 是不能填 2、3、4、8 的任意一个数的。
...
和前文推理完全一样，照着搬下来就行。先是假设基准单元格，然后得到其中一边的目标单元格是 $$a$$ 和 $$b$$ 后，随即得到另外一个飞鱼结构的基准单元格只能是 $$c$$ 和 $$d$$，然后得到另外一个飞鱼的目标单元格也是 $$c$$ 和 $$d$$，最终得到跨区四数组的结论。
...
## 双飞鱼和飞鱼 4 的名称撞了 <a href="#naming-conflict-on-exocet-four" id="naming-conflict-on-exocet-four"></a>

在飞鱼理论里，之前我们简要提过，飞鱼按目标单元格数量来取名。但是很明显，这个例子其实是两个飞鱼结构拼一起用的，但是不巧的是，老外把它命名成了飞鱼 4（缩写 JE4）。这和之前有 4 个目标单元格的名称冲突了。

不过这不打紧。我们很少会出现标准飞鱼里，一个飞鱼用两个共轭对的这种 JE4 类型，所以一般很难在这上面引起歧义，不过这确实是一个问题。
