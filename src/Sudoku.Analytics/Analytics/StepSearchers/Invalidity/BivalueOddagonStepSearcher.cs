			...
			for (var size = BitOperations.PopCount((uint)otherDigitsMask) - 1; size < otherCells.Count; size++)
			{
				foreach (ref readonly var cells in otherCells & size)
				{
					...
					var step = new BivalueOddagonType3Step(
						...
						[[.. candidateOffsets, new HouseViewNode(ColorIdentifierAlias.Normal, house), .. links]],
						context.Options,
						loop,
						d1,
						d2,
		...
		var step = new BivalueOddagonType4Step(
			...
			[
				[
					.. candidateOffsets,
					.. links,
					new ConjugateLinkViewNode(ColorIdentifierAlias.Normal, extraCellsMap[0], extraCellsMap[1], extraDigit)
				]
			],
			context.Options,
			loop,
