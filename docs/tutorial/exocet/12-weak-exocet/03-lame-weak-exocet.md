...
{% hint style="warning" %}
本文的编号为 03。原本编号为 02 的文章是预留给了对衰弱飞鱼的删数的完整证明的，但是证明的难度颇高，要写成文档需要大幅度制图和说明文字。本周因为我的时间不够，所以暂时没有完整梳理出证明过程并转为文档描述，所以我们暂时跳过证明的文档内容；以后有空了我会把证明补到缺失的 02 编号的位置上。
{% endhint %}
...
我们就把这种衰弱飞鱼还缺少强心针单元格的特殊飞鱼类型称为全衰弱飞鱼，简称**全衰飞鱼**。
